...
function renderQueue(control) {
  ...
  const recordCards = records.filter(item => !groupedIds.has(item.id)).slice().reverse().map(item => {
    ...
    const launcherVerified = tested && item.post_publish_evidence?.player_launcher?.pass === true;
